...
function resolveStatusMeta({ isLoading, error, items }) {
  if (isLoading) {
    return {
      label: "Загрузка",
      className: "border-cyan-300/28 bg-cyan-300/[0.12] text-cyan-50",
    };
  }
  ...
  if (error) {
    return {
      label: "Ошибка",
      className: "border-rose-300/30 bg-rose-500/[0.14] text-rose-100",
    };
  }
  ...
  if (items.length > 0) {
    return {
      label: "Загружено",
      className: "border-emerald-300/30 bg-emerald-400/[0.14] text-emerald-100",
    };
  }

  return {
    label: "Нет данных",
    className: "border-slate-300/20 bg-slate-500/[0.12] text-slate-200",
  };
}
  ...
  const currentSourceDataset = useMemo(() => {
    if (activeSourceTab === IMPORT_CONTEXT_INVENTORY_BALANCE) {
      return {
        ...
        title: "Остатки",
        subtitle: "Проверяйте доступный остаток по номенклатуре перед запуском расчёта потребности.",
      ...
      dateInputType: "select_dates",
        ...
        hints: [
          "Импортируйте остатки в той же единице измерения, что и у номенклатуры.",
          "После загрузки используйте поиск, чтобы быстро проверить критичные позиции.",
          "Остатки являются снимком склада на конкретную дату. По умолчанию выбирается последняя загруженная дата.",
        ],
        checks: [
    ...
    if (activeSourceTab === IMPORT_CONTEXT_SAFETY_STOCK) {
      return {
        ...
        title: "Страховой запас",
        subtitle: "Поддерживайте актуальные минимальные запасы, чтобы расчёт выявлял реальные дефициты.",
        dateLabel: null,
        dateInputType: null,
        dateValue: "",
        onDateChange: () => {},
        ...
        hints: [
          "Страховой запас нужен для стабильности плана между пересчётами.",
          "Если позиция отсутствует, она не участвует в ограничении потребности.",
        ],
        checks: [
    ...
    return {
      ...
      title: "План продаж",
      subtitle: "Выберите период планирования и проверьте входные объёмы до запуска расчёта потребности.",
      ...
      hints: [
        "Загружайте план продаж на период, по которому будете считать потребность.",
        "Проверяйте позиции с большими объёмами через поиск перед запуском расчёта.",
      ],
      checks: [
  ...
  return (
    <section className="space-y-6">
      ...
      {activeModuleTab === MODULE_TAB_SOURCE_DATA ? (
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.26fr)_minmax(0,0.74fr)] 2xl:grid-cols-[minmax(0,1.32fr)_minmax(0,0.68fr)]">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-4 space-y-1 text-sm text-slate-300">
                      ...
                      {card.dateBadgeText ? (
                        <div className="text-[11px] uppercase tracking-[0.14em] text-cyan-200/75">
                          {card.dateBadgeText}
                        </div>
                      ) : null}
                    </div>
            ...
            <section className="glass-panel p-4 sm:p-5">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                <div className="max-w-2xl">
                  <h2 className="font-['Space_Grotesk'] text-2xl font-semibold text-slate-50">
                    {currentSourceDataset.title}
                  </h2>
                  <p className="mt-2 text-sm leading-6 text-slate-300">{currentSourceDataset.subtitle}</p>
                </div>

                <div className="grid w-full gap-3 sm:grid-cols-2 xl:w-auto xl:grid-cols-[minmax(0,220px)_minmax(0,260px)_auto]">
                    ...
                    <div>
                      ...
                      {currentSourceDataset.dateInputType === "select_dates" ? (
                        <select
                          ...
                          className="w-full rounded-none border border-white/[0.08] bg-[linear-gradient(180deg,rgba(16,30,43,0.76),rgba(9,17,27,0.9))] px-3 py-2.5 text-sm text-slate-100 outline-none transition focus:border-cyan-300/45 disabled:cursor-not-allowed disabled:opacity-60"
                        >
                          {currentSourceDataset.dateOptions.length > 0 ? (
                      ...
                      {currentSourceDataset.dateInputType === "select_dates" ? (
                        <div className="mt-2 text-xs text-slate-400">
                          Для расчёта по умолчанию используется последняя загруженная дата остатков.
                        </div>
                      ) : null}
                    </div>
                  ) : (
                    <div className="flex items-end text-sm text-slate-400">Фильтр даты не требуется</div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center border border-white/[0.08] bg-[linear-gradient(180deg,rgba(16,30,43,0.76),rgba(9,17,27,0.9))] px-3 py-2.5">
                      ...
                      <input
                        ...
                        placeholder="По коду или наименованию"
                  ...
                  <div className="flex items-end gap-2">
                    {activeSourceTab === IMPORT_CONTEXT_SALES_PLAN ? (
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-none border border-cyan-400/30 bg-cyan-400/14 px-4 py-2.5 text-sm font-medium text-cyan-50 transition hover:bg-cyan-400/[0.18] disabled:cursor-not-allowed disabled:opacity-60"
                      >
                    ...
                    <button
                      ...
                      className="inline-flex w-full items-center justify-center gap-2 rounded-none border border-white/12 bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-slate-200 transition hover:border-cyan-400/20 hover:bg-cyan-400/[0.07] disabled:cursor-not-allowed disabled:opacity-60 xl:w-auto"
                    >
                      <RefreshCw
                    ...
                    <table className="min-w-full border-collapse">
                      <thead className="sticky top-0 z-10 bg-[linear-gradient(180deg,rgba(19,39,56,0.95),rgba(14,28,40,0.96))]">
                        <tr className="text-left">
                          <th className="px-3 py-2 text-[11px] uppercase tracking-[0.14em] text-slate-400">
                            Код
                          </th>
                          <th className="px-3 py-2 text-[11px] uppercase tracking-[0.14em] text-slate-400">
                            Наименование
                      ...
                      <tbody>
                        {currentSourceDataset.filteredItems.map((item) => (
                          <tr
                            ...
                            className="border-t border-white/[0.05] transition hover:bg-cyan-400/[0.05]"
                          >
                      ...
                      <tfoot>
                        <tr className="border-t border-cyan-300/14 bg-cyan-400/[0.06]">
                          <td className="px-3 py-2 text-xs uppercase tracking-[0.14em] text-slate-300">Итого</td>
          ...
          {isImportOpen ? (
            ...
            <aside className="glass-panel h-fit p-5 sm:p-6 xl:sticky xl:top-6">
              ...
              <div className="mt-5 space-y-4">
                <div>
                  <div className="mb-2 text-xs uppercase tracking-[0.18em] text-slate-500">Дата остатков</div>
                  <input
                    type="text"
                    readOnly
                    value={balanceDate || "—"}
                ...
                <div>
                  <div className="mb-2 text-xs uppercase tracking-[0.18em] text-slate-500">Доступный остаток</div>
                  <input
                    type="number"
                    step="0.001"
                    min="0"
            ...
            <aside className="glass-panel h-fit p-5 sm:p-6 xl:sticky xl:top-6">
              ...
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <div className="mb-2 text-xs uppercase tracking-[0.18em] text-slate-500">Страховой запас</div>
                  <input
                    type="number"
                    step="0.001"
                    min="0"
            ...
            <aside className="glass-panel h-fit p-5 sm:p-6 xl:sticky xl:top-6">
              ...
              <div className="mt-6 grid gap-3">
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-none border border-cyan-400/30 bg-cyan-400/14 px-4 py-2.5 text-sm font-medium text-cyan-50 shadow-cyanGlow transition hover:bg-cyan-400/18"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-none border border-white/12 bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-slate-200 transition hover:border-cyan-400/20 hover:bg-cyan-400/[0.07]"
                >
              ...
              {activeSourceTab === IMPORT_CONTEXT_SALES_PLAN ? (
                <div className="mt-4 rounded-none border border-white/10 bg-white/[0.03] px-4 py-4">
                  <div className="text-sm font-medium text-slate-100">Ручная корректировка</div>
                  <p className="mt-2 text-sm leading-6 text-slate-300">
                    Для точечных изменений можно добавить позицию вручную, изменить количество или удалить лишнюю строку.
                  </p>
                </div>
              ) : null}
              {activeSourceTab === IMPORT_CONTEXT_INVENTORY_BALANCE ? (
                <div className="mt-4 rounded-none border border-white/10 bg-white/[0.03] px-4 py-4">
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-300">
                    Для точечных изменений можно добавить позицию вручную, изменить доступный остаток или удалить лишнюю строку из выбранного снимка.
                  </p>
                  {!balanceDate ? (
                    <p className="mt-2 text-xs text-amber-200/80">Сначала загрузите или выберите дату остатков.</p>
                  ) : null}
                </div>
              ) : null}
              {activeSourceTab === IMPORT_CONTEXT_SAFETY_STOCK ? (
                <div className="mt-4 rounded-none border border-white/10 bg-white/[0.03] px-4 py-4">
                  ...
                  <div className="mt-3">
                    <button
                      type="button"
                      onClick={handleOpenCreateSafetyStockForm}
                      className="inline-flex items-center justify-center gap-2 rounded-none border border-cyan-400/30 bg-cyan-400/14 px-4 py-2.5 text-sm font-medium text-cyan-50 transition hover:bg-cyan-400/[0.18]"
                    >
                      <Plus className="h-4 w-4" />
                      Добавить позицию
                    </button>
                  ...
                  <p className="mt-3 text-sm leading-6 text-slate-300">
                    Для точечных изменений можно добавить позицию вручную, изменить страховой запас или удалить лишнюю строку.
                  </p>
                </div>
              ) : null}
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="rounded-none border border-white/10 bg-white/[0.03] px-4 py-4">
                  <div className="text-sm font-medium text-slate-100">Подсказки</div>
                ...
                <div className="rounded-none border border-white/10 bg-white/[0.03] px-4 py-4">
                  ...
                  <div className="mt-3 flex items-start gap-2 text-sm text-emerald-100">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0" />
                    <span>
                      {currentSourceDataset.error
                        ? "Есть ошибка загрузки, проверьте источник данных или повторите импорт."
                        : currentSourceDataset.isLoading
                          ? "Данные обновляются, проверка будет доступна после загрузки."
                          : "Набор данных загружен, можно переходить к расчёту потребности."}
                    </span>
                  </div>
                  <div className="mt-3 space-y-2 text-sm leading-6 text-slate-300">
                    {currentSourceDataset.checks.map((check) => (
                      <p key={check}>{check}</p>
                    ))}
                  </div>
                </div>
      ...
      {activeModuleTab === MODULE_TAB_CALCULATE ? (
        <section className="glass-panel p-6 sm:p-7">
          <div className="panel-title">Расчёт</div>
          <h2 className="mt-3 font-['Space_Grotesk'] text-2xl font-semibold text-slate-50">
            Расчёт потребности
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
            Здесь будет запуск расчёта потребности через `/demand/calculate` с параметрами периода,
            горизонта и выбранного набора входных данных.
          </p>
        </section>
      ) : null}
      ...
      {activeModuleTab === MODULE_TAB_RESULTS ? (
        <section className="glass-panel p-6 sm:p-7">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-6 text-slate-300">
            Здесь будет просмотр верхнего спроса, внутренней потребности, внешней потребности и
            проблем расчёта с фильтрами и переходом к детализации по позициям.
          </p>
        </section>
      ) : null}
    </section>
