  ...
  return (
    <section className="glass-panel min-h-[560px] p-5 sm:p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="panel-title">Технологические операции</div>
      ...
      <div className="mt-5 space-y-3">
        {isLoading ? (
          <div className="border border-white/[0.05] bg-[linear-gradient(180deg,rgba(16,26,37,0.58),rgba(10,18,27,0.7))] px-5 py-8 text-center">
            <div className="text-base font-medium text-slate-200">Загрузка операций...</div>
          </div>
        ) : items.length > 0 ? (
          items.map((item) => {
            const isSelected = item.process_id === selectedItemId;
            ...
            return (
              <button
                key={item.process_id}
                type="button"
                onClick={() => onSelectItem(item.process_id)}
                className={[
                  ...
                  isSelected
                    ...
                    : isInactive
                      ? "border-amber-200/[0.16] bg-[linear-gradient(180deg,rgba(49,35,20,0.34),rgba(19,14,10,0.52))] hover:border-amber-200/[0.30] hover:bg-[linear-gradient(180deg,rgba(62,44,26,0.42),rgba(24,18,12,0.58))] hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.03),0_0_0_1px_rgba(251,191,36,0.08)]"
              ...
              >
                ...
                <div
                  className={[
                    ...
                    isSelected
                      ? isInactive
                        ? "border-amber-200/42 bg-amber-200/16 text-amber-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]"
                        : "border-cyan-200/45 bg-cyan-300/18 text-cyan-50 shadow-[0_0_24px_rgba(34,211,238,0.18)]"
                      : isInactive
                        ? "border-amber-200/20 bg-amber-950/36 text-amber-200/72 group-hover:border-amber-200/34 group-hover:text-amber-100/88"
                ...
                <div className="relative z-10 min-w-0 flex-1">
                  <div className="flex flex-wrap items-center gap-3">
                    <span
                      className={[
                        ...
                        item.is_active ? "text-slate-50" : "text-slate-300",
                      ].join(" ")}
                    >
                      {item.process_code}
                    </span>
                  ...
                  <div
                    className={[
                      "mt-2 text-base",
                      item.is_active ? "text-slate-200" : "text-slate-400",
                    ].join(" ")}
                  >
                    {item.process_name}
                  </div>
                  ...
                  className={[
                    ...
                    isSelected
                      ? item.is_active
                        ...
                        : "translate-x-1 text-amber-100/92"
                      : item.is_active
                        ? "text-slate-600 group-hover:translate-x-1 group-hover:text-cyan-100/70"
                        : "text-amber-200/34 group-hover:translate-x-1 group-hover:text-amber-100/70",
                  ].join(" ")}
                />
              </button>
            );
          ...
          <div className="border border-white/[0.05] bg-[linear-gradient(180deg,rgba(16,26,37,0.58),rgba(10,18,27,0.7))] px-5 py-8 text-center">
            <div className="text-base font-medium text-slate-200">Операции не найдены</div>
            <div className="mt-2 text-sm text-slate-500">
              Попробуйте изменить код или наименование в строке поиска.
            </div>
          </div>
        )}
      </div>
