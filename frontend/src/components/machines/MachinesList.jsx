      ...
      <div className="mt-5 space-y-3">
        {isLoading ? (
          <div className="border border-white/[0.05] bg-[linear-gradient(180deg,rgba(16,26,37,0.58),rgba(10,18,27,0.7))] px-5 py-8 text-center">
            <div className="text-base font-medium text-slate-200">Загрузка оборудования...</div>
          </div>
        ) : items.length > 0 ? (
          items.map((item) => {
            const isSelected = item.machine_id === selectedItemId;
            ...
            return (
              <button
                key={item.machine_id}
                type="button"
                onClick={() => onSelectItem(item.machine_id)}
                className={[
